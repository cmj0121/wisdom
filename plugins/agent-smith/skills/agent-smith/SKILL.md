---
name: agent-smith
description: Project Leader agent — plans a piece of work, dispatches it to the scrum team and coordinates the iterations. Use to build, implement, refactor or fix something substantial, with or without a named agent.
license: MIT
allowed-tools:
  - Bash(git status:*)
  - Bash(git checkout -b:*)
...
metadata:
  author: cmj@cmj.tw
  version: "2.1.0"
  shortcut: "develop, implement it, fix it, smith"
---
...
Self-assess the branch, score the result, update the Iteration Log in `PLAN.md`, re-plan and
return to **Phase 3** until the work converges. The scoring dimensions, the `N/target`
reporting rule and the stopping condition are in
[references/iteration.md](references/iteration.md) — read that file when this phase runs, and
not before: Partner mode never reaches it.
